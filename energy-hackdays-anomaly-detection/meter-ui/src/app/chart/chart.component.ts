...
export class ChartComponent implements OnDestroy, AfterViewInit {
  ...
  public triggerSimulation(): void {
    ...
    console.log("Starting simulation for meter:", this.meterId);

    // Send 10 normal data points quickly as a batch (reduced from 210 for UI responsiveness)
    // The Python model threshold might need adjustment or we send enough to trigger it
    const points = 210;
    let sent = 0;

    const sendPoint = (kwh: number) => {
      ...
      this.dataService.simulate(payload).subscribe({
        next: (anomalies) => {
          sent++;
          if (sent < points) {
            sendPoint(0.5);
          } else if (sent === points) {
            // Last point is the anomaly
            sendPoint(500.0);
          } else {
