...
export default function Analytics() {
  ...
  const dailyData = useMemo(() => {
    ...
    for (const s of samples) {
      ...
      if (!current) {
        byDay.set(key, { min: s.energy, max: s.energy });
      } else {
        byDay.set(key, { min: Math.min(current.min, s.energy), max: Math.max(current.max, s.energy) });
      }
    }
    return Array.from(byDay.entries()).map(([date, { min, max }]) => {
      const energy = max - min;
      return { date, energy, cost: energy * 8 };
    });
  }, [samples]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {samples.length > 0
                ? (samples[samples.length - 1].energy - samples[0].energy).toFixed(3)
                : "0.000"} kWh
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {dailyData.length && samples.length > 0
                ? ((samples[samples.length - 1].energy - samples[0].energy) / dailyData.length).toFixed(3)
                : "0.000"} kWh
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Peak Daily Energy</CardTitle>
            <TrendingUp className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            {(() => {
              const maxEnergyDay = dailyData.reduce((max, d) => d.energy > max.energy ? d : max, dailyData[0] || { energy: 0, date: new Date().toISOString() });
              return (
                <>
                  <div className="text-2xl font-bold">{maxEnergyDay.energy.toFixed(3)} kWh</div>
                  <p className="text-xs text-muted-foreground">
                    {dailyData.length > 0 ? new Date(maxEnergyDay.date).toLocaleDateString() : 'No data'}
                  </p>
                </>
              );
            })()}
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>AI Energy Prediction (BiLSTM)</CardTitle>
